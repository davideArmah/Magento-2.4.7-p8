...
namespace Armah\Orderattr\Model\Value\Metadata;

use Armah\Orderattr\Model\Attribute\Attribute;
use Armah\Orderattr\Model\Attribute\RelationValidator;
use Armah\Orderattr\Model\Attribute\Validator\ValidatorPool;
use Armah\Orderattr\Model\Config\Source\CheckoutStep;
use Armah\Orderattr\Model\ResourceModel\Attribute\Relation\RelationDetails\CollectionFactory
    as RelationDetailsCollectionFactory;
use Armah\Orderattr\Model\Value\Metadata\Form\CollectionFilter\FilterPool;
use Magento\Framework\App\ObjectManager;
use Magento\Framework\App\RequestInterface;

/**
 * @method Attribute[] getAllowedAttributes()
 * @method Attribute[] getAttributes()
 * @method Attribute|bool getAttribute($attributeCode)
 */
class Form extends \Magento\Eav\Model\Form
{
    public const FORMAT_TO_VALIDATE_RELATIONS = 'armah_relations';
     ...
     * @var string
     */
    protected $_moduleName = 'Armah_Orderattr';
     ...
     * @var string
     */
    protected $_entityTypeCode = \Armah\Orderattr\Model\ResourceModel\Entity\Entity::ENTITY_TYPE_CODE;
    ...
    public function __construct(
        ...
        \Armah\Orderattr\Model\ResourceModel\Attribute\CollectionFactory $attributeCollectionFactory,
        ...
        ?ValidatorPool $validatorPool = null //todo: move to not optional
    ) {
        parent::__construct(
            $storeManager,
            $eavConfig,
        ...
        $this->validatorPool = $validatorPool ?? ObjectManager::getInstance()->get(ValidatorPool::class);
    }
    ...
    public function getEntity()
    {
        if ($this->_entity === null) {
            $this->_entity = $this->_universalFactory->create(\Armah\Orderattr\Model\Entity\EntityData::class);
        }
     ...
     * @return \Armah\Orderattr\Model\ResourceModel\Attribute\Collection
     */
    protected function _getFilteredFormAttributeCollection()
    {
        ...
        if ($this->_ignoreInvisible) {
            if ($this->_store) {
                $collection->addStoreFilter((int)$this->_store->getId());
            }

            if ($this->getCustomerGroupId() !== null) {
                $collection->addCustomerGroupFilter((int)$this->getCustomerGroupId());
            }
    ...
    private function isValid(Attribute $attribute): bool
    {
