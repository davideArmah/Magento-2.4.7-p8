...
use Armah\Orderattr\Model\Indexer\FlatScopeResolver;
...
{
    public const INDEX_SIZE_LIMIT = 1024;
    ...
    protected function createFlatTable($tableName, array $fields)
    {
        ...
        foreach ($fields as $field) {
            ...
            $type = (string)$columnMap['type'];
            $size = (int)$columnMap['size'];
            if ($field['type'] === 'filterable') {
                $indexInfo = ['name' => $name];
                if (in_array($type, [Table::TYPE_TEXT, Table::TYPE_BLOB], true)) {
                    $size = min($size, self::INDEX_SIZE_LIMIT);
                    if ($size > 0) {
                        $indexInfo['size'] = $size;
                    }
                }
