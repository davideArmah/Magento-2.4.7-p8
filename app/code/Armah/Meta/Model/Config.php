...
class Config extends \Magento\Framework\Model\AbstractModel implements ConfigInterface
{
    ...
    {
        parent::_construct();
        $this->_init(\Armah\Meta\Model\ResourceModel\Config::class);
    }
    ...
    public function ifStoreConfigExists(\Armah\Meta\Model\Config $item)
    {
    ...
    public function getRecursionConfigData($paths, $storeId)
    {
        ...
        if (isset($result[0]) && is_object($result[0])) {
            $applied = $this->_registry->registry('armeta_applied_rule');
            if (!is_array($applied)) {
                $applied = [];
            }
            $applied[] = __('Template (%1) #%2', __('Category'), $result[0]->getId());
            $this->_registry->unregister('armeta_applied_rule');
            $this->_registry->register('armeta_applied_rule', $applied);
        }
