...
namespace Armah\Meta\Helper;

use Armah\Meta\Model\Config;
use Armah\Meta\Model\Integration\IntegrationFactory;
use Armah\Meta\Model\Meta\ReplacedData;
...
class Data extends AbstractHelper
{
    ...
    public const CONFIG_MAX_META_DESCRIPTION = 'armeta/general/max_meta_description';
    public const CONFIG_MAX_META_TITLE = 'armeta/general/max_meta_title';
    ...
    public function __construct(
        ...
        \Armah\Meta\Model\Config $metaConfig,
     ...
     * Get config by url
     *
     * @return \Armah\Meta\Model\Config
     */
    ...
    protected function prepareData($data, $customUrlMapping)
    {
        foreach ($data as $item) {
            if (isset($item['config_id'])) {
                $applied = $this->registry->registry('armeta_applied_rule');
                if (!is_array($applied)) {
                    $applied = [];
                }
                $applied[] = __('Template (%1) #%2', __('Url'), $item['config_id']);
                $this->registry->unregister('armeta_applied_rule');
                $this->registry->register('armeta_applied_rule', $applied);
            }
     ...
     */
    public function parse(?string $tpl, bool $isUrl = false, ?int $entityId = null)
    {
        if (!$tpl) {
            return null;
        }
        ...
        $tpl = $this->_parseAttributes($tpl, $isUrl, $entityId);
     ...
     * @return mixed
     */
    protected function _parseAttributes(string $tpl, bool $isUrl, ?int $entityId = null)
    {
        $vars = [];
        ...
        foreach ($vars as $codes) {
            ...
            foreach (explode('|', $codes) as $code) {
                foreach ($this->_entityCollection as $object) {
                    if ($entityId && $entityId !== (int)$object->getId()) {
                        continue;
                    }
    ...
    protected function getCategoryValue($p)
    {
        ...
        if ($this->getConfig('armeta/product/no_breadcrumbs')) {
    ...
    {
        $domQuery = new \Laminas\Dom\Query($html);
    ...
    public function getReplaceData($code)
    {
        ...
        if (isset($currentCategory) && !isset($currentProduct)) {
            ...
            $forceOverwrite = $this->scopeConfig->isSetFlag('armeta/cat/force');
        } elseif (isset($currentProduct)) {
            $currentEntity = $currentProduct;
            $forceOverwrite = $this->scopeConfig->isSetFlag('armeta/product/force');
        }
    ...
    {
        if (!$product || !$this->getConfig('armeta/product/enabled') || !$product->getCategoryIds()) {
            return false;
        }
        ...
        foreach ($attributes as $attrCode) {
            ...
            if ($configItem) {
                ...
                $tag = $this->parse($configItem, false, (int)$product->getId());

                $max = (int) $this->getConfig('armeta/general/max_' . $attrCode);
                if ($max) {
    ...
    /**
     * @return \Armah\ShopbyBrand\Helper\Content|null
     */
    private function getBrandContentHelper()
    {
        return $this->integrationFactory->get(\Armah\ShopbyBrand\Helper\Content::class);
    }
}
