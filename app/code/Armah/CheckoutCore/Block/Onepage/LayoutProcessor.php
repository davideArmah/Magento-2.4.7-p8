...
use Armah\CheckoutCore\Model\Amazon\Config as AmazonConfig;
use Armah\CheckoutCore\Model\CheckoutConfigProvider\Gdpr\ConsentsProvider;
use Armah\CheckoutCore\Model\Config;
use Armah\CheckoutCore\Model\CustomerAttribute\Storage;
use Armah\CheckoutCore\Model\LayoutProcessor\SortFields;
...
{
    public const GDPR_COMPONENT_NAME = 'armah-gdpr-consent';
    ...
    public function process($jsLayout)
    {
        ...
        $this->walker->setValue(
            ...
            'Armah_CheckoutCore/js/view/checkout/summary/item/details'
        );

        $this->walker->setValue(
            '{CART_ITEMS}.component',
            'Armah_CheckoutCore/js/view/checkout/summary/cart-items'
        );
        ...
        $this->walker->unsetByPath('{SHIPPING_ADDRESS}.>>.before-fields.>>.order-attributes-fields-default');
        ...
        if ($paymentListComponent) {
            ...
            if ($checkedAgreement) {
                ...
                $this->walker->setValue(
                    ...
                    'Armah_CheckoutCore/js/view/validators/agreement-validation'
                );
            }
        }
    }
    ...
    /**
     * The method process Armah_Gdpr module checkboxes
     */
    private function processGdprCheckboxes(): void
    {
        ...
        if ($gdprComponent && $this->moduleEnable->isGdprEnable()) {
            ...
            if (!empty($consentsConfig) && !empty($oneStep)) {
                ...
                $gdprComponent['component'] = 'Armah_CheckoutCore/js/view/gdpr-consent';
                unset($gdprComponent['displayArea']);
                $oneStep['config']['gdprComponentTmpl'] = $gdprComponent;
                $oneStep['config']['gdprTml'] = 'Armah_CheckoutCore/onepage/gdpr-container';
