...
namespace Armah\CheckoutCore\Plugin\Checkout\Block\Checkout;
...
class AttributeMerger
{
    ...
    public function beforeMerge(
        ...
        if (isset($elements['telephone'])) {
            $fields['telephone']['config']['elementTmpl'] = 'Armah_CheckoutCore/form/element/telephone';
            $fields['telephone']['config']['tooltipTpl'] = 'Armah_CheckoutCore/form/element/tooltip';
        }

        if (in_array('armah_checkout', $this->layout->getUpdate()->getHandles())) {
    ...
    private function setDefaultRegionData(array &$config): void
    {
        if (isset($config['region_id'])) {
            $defaultData = $this->getDefaultData();
            $config['region_id']['component'] = 'Armah_CheckoutCore/js/form/element/region';
    ...
    /**
     * @param array $attributeConfig
     * @param \Armah\CheckoutCore\Model\Field $field
     *
     * @return mixed
     */
