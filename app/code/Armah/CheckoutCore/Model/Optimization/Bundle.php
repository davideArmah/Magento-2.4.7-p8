...
class Bundle implements BundleInterface
{
     ...
     */
    public const ROOT_BUNDLE_JS_DIR = 'armah/bundle';
    ...
    private $excludeFiles = [
        BundleService::COLLECT_SCRIPT_PATH
    ];
        ...
        foreach ($this->files as $contentPoolName => $files) {
            ...
            foreach ($files as $filePath => $sourcePath) {
                try {
                    ...
                    $this->logger->warning(
                        sprintf(
                            'Checkout bundling process. One of the files are skipped due error: %s',
                            $e->getMessage()
                        ),
                        ['exception' => $e]
                    );
            ...
            try {
                ...
                $this->logger->error(
                    sprintf('Checkout bundling error: %s', $e->getMessage()),
                    ['exception' => $e]
                );
            }
        }
