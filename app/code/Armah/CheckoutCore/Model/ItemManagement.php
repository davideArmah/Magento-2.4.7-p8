...
class ItemManagement implements ItemManagementInterface
{
    ...
    public function update($cartId, $itemId, $formData)
    {
        ...
        return $this->totalsFactory->create(['data' => [
            'totals' => $items,
            'payment' => $this->paymentMethodManagement->getList($cartId),
            'quote_messages' => $this->getQuoteMessages($item)
        ]]);
    }
     ...
     * @return string
     */
    private function getQuoteMessages($item)
    {
        $quoteMessages = [];
        $quoteMessages[$item->getId()] = $item->getMessage() ?: '';

        return json_encode($quoteMessages);
    }
}
