...
namespace Armah\CheckoutCore\Test\Unit\Model\Field\ConfigManagement\CustomerAttributes;
    ...
    public function testExecuteWithWebsite(bool $isEnabled, bool $isRequired, bool $canChangeRequiredValue): void
    {
        ...
        if ($canChangeRequiredValue) {
            $this->attributeMock
                ...
                ->method('setData')
                ->willReturnCallback(function ($key, $value) use ($isEnabled, $isRequired) {
                    if ($key === 'scope_is_visible') {
                        $this->assertEquals($isEnabled, $value);
                    } elseif ($key === 'scope_is_required') {
                        $this->assertEquals($isRequired, $value);
                    }
