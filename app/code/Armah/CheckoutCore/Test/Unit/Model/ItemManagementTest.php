...
use Armah\CheckoutCore\Model\ItemManagement;
use Armah\CheckoutCore\Model\Totals;
use Armah\CheckoutCore\Model\TotalsFactory;
...
class ItemManagementTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $totalsFactoryMock->method('create')->willReturn($totalsMock);
    ...
    public function testRemove()
    {
        ...
        $quote->method('isVirtual')->will($this->onConsecutiveCalls(true, false, true));
        $quote->method('getItemById')->willReturn($quoteItem);
        $quoteItem->method('getId')->will($this->onConsecutiveCalls(1, 0));

        $this->assertInstanceOf(Totals::class, $this->model->remove(1, 2, $this->address));
    ...
    public function testUpdateWithoutItem()
    {
        ...
        $quote->method('isVirtual')->will($this->onConsecutiveCalls(true, true , false, true));
        $quote->method('getItemById')->willReturn(false);

        $this->assertInstanceOf(Totals::class, $this->model->update(1, 2, ''));
    }
    ...
    public function testUpdateWithStringItem()
    {
        ...
        $this->cartRepository->method('get')->willReturn($quote);
        $this->cart->method('getQuote')->willReturn($quote);
        $this->cart->method('updateItem')->willReturn('test');
        $shippingAddress = $this->createAddressMock('shipping');
        $billingAddress = $this->createAddressMock('billing');
        $quote->method('getShippingAddress')->willReturn($shippingAddress);
        $quote->method('getBillingAddress')->willReturn($billingAddress);
        $quote->method('isVirtual')->will($this->onConsecutiveCalls(true, true , false, true));
        $quote->method('getItemById')->willReturn($quote);
    ...
    public function testUpdateWithErrorInItem()
    {
        ...
        $quote->method('getShippingAddress')->willReturn($shippingAddress);
        $quote->method('getBillingAddress')->willReturn($billingAddress);
        $this->cart->method('updateItem')->willReturn($itemMock);
        $quote->method('isVirtual')->willReturn(true, true , false, true);
    ...
    public function testUpdate()
    {
        ...
        $item = $this->createPartialMock(\Magento\Quote\Model\Quote\Item::class, ['clearMessage']);

        $this->cartRepository->method('get')->willReturn($quote);
        $this->cart->method('getQuote')->willReturn($quote);
        $this->cart->method('updateItem')->willReturn($quote);
        $this->cart->expects($this->exactly(2))->method('save');
        $shippingAddress = $this->createAddressMock('shipping');
        $billingAddress = $this->createAddressMock('billing');
        $quote->method('getShippingAddress')->willReturn($shippingAddress);
        $quote->method('getBillingAddress')->willReturn($billingAddress);
        $quote->method('isVirtual')->willReturn(false, true, true, true);
        $quote->method('getItemById')->willReturn($item);
        $item->method('clearMessage')->willReturn($item);
        $quote->method('getAllVisibleItems')->willReturn([]);
        $quote->method('getItems')->willReturn([0 => []]);
