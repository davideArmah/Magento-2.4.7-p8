...
namespace Armah\Base\Observer;

use Armah\Base\Model\ArmahMenu\Frontend\Processors\Links;
 ...
 * @deprecated
 * @since 1.12.18
 * @see \Armah\Base\Block\Adminhtml\System\Config\Information
 */
class GenerateInformationTab implements ObserverInterface
{
    ...
    public const FEATURE_LINK = 'https://armah.com/request-a-feature';
    ...
    {
        $html = '<div class="armah-info-block">'
    ...
    {
        $src = $this->assetRepo->getUrl('Armah_Base::images/armah_logo.svg');
        if ($this->moduleInfoProvider->isOriginMarketplace()) {
            $href = Links::MAGENTO_MARKET_URL;
        } else {
            $href = '' . $this->getSeoParams() . 'armah_logo_' . $this->getModuleCode();
        }

        $html = '<a target="_blank" href="' . $href . '"><img class="armah-logo" src="' . $src . '"/></a>';
        ...
        if ($content) {
            ...
            foreach ($content as $message) {
                if (isset($message['type'], $message['text'])) {
                    $html .= '<div class="armah-additional-content"><span class="message ' . $message['type'] . '">'
                        . $message['text']
                        . '</span></div>';
                }
            }
    ...
    {
        $html = '<div class="armah-module-version">';

        $currentVer = $this->getCurrentVersion();
        if ($currentVer) {
            $html .= '<div><span class="version-title">'
                . $this->getModuleName() . ' '
                . '<span class="module-version">' . $currentVer . '</span>'
                . '</span>'
                . '</div>';
        }
    ...
    {
        $html = '<div class="armah-user-guide"><span class="message success">'
            . __(
    ...
    {
        return '<div class="arbase-buttons-container">'
            . $this->getFeatureLink()
    ...
    private function showModuleExistingConflicts()
    {
        ...
        if (!empty($messages)) {
            $html = '<div class="armah-conflicts-title">'
                . __('Problems detected:')
                . '</div>';

            $html .= '<div class="armah-disable-extensions">';
