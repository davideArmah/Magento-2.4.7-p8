...
class RecurringData implements InstallDataInterface
{
    ...
    public const IS_ARMAH_COLUMN = 'is_armah';
     ...
     * Consider refactoring to store armah columns data in separate table.
     */
    private function processNotificationTable(ModuleDataSetupInterface $setup): void
    {
        if ($setup->getConnection()->isTableExists($setup->getTable(self::NOTIFICATION_TABLE))) {
            if (!$this->notificationTableColumnExist($setup, self::IS_ARMAH_COLUMN)) {
                $this->addIsArmahField($setup);
            }
    ...
    private function addIsArmahField(ModuleDataSetupInterface $setup): void
    {
        $setup->getConnection()->addColumn(
            $setup->getTable(self::NOTIFICATION_TABLE),
            self::IS_ARMAH_COLUMN,
            [
                'type' => Table::TYPE_SMALLINT,
                'nullable' => false,
                'default' => 0,
                'comment' => 'Is Armah Notification'
            ]
        );
    }
