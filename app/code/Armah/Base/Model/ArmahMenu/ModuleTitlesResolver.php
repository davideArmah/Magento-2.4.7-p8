...
class ModuleTitlesResolver
{
    ...
    /**
     * Get installed Armah modules titles for menu building
     *
    ...
    {
        $armahModules = array_intersect($this->getArmahModules(), array_keys($menuItems));

        $modules = [];
        foreach ($armahModules as $moduleCode) {
    ...
    public function getModuleTitle(string $moduleCode): string
    {
        ...
        if ($module && isset($module['name'])) {
            ...
            $title = str_replace('Armah_', '', $moduleCode);
    ...
    /**
     * Retrieve all installed armah module names
     *
     * @return array
     */
    private function getArmahModules(): array
    {
        ...
        return array_filter($this->moduleList->getNames(), function ($moduleCode) use ($restrictedModules) {
            return $moduleCode !== 'Armah_Base'
                && strpos($moduleCode, 'Armah_') !== false
