...
namespace Armah\Base\Model\Feed\FeedTypes;

use Armah\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\ExistsFactory;
use Armah\Base\Model\Config;
use Armah\Base\Model\Feed\FeedContentProvider;
use Armah\Base\Model\FlagsManager;
use Armah\Base\Model\ModuleInfoProvider;
use Armah\Base\Model\Parser;
...
class News
{
    ...
    public function execute(): array
    {
        ...
        // Feed disabled - parseXml returns false, skip processing
        if ($feedXml !== false && isset($feedXml->channel->item)) {
    ...
    private function isItemValid(\SimpleXMLElement $item): bool
    {
        return $this->validateByExtension((string)$item->extension)
            && $this->validateByArmahCount($item->armah_module_qty)
            && $this->validateByNotInstalled((string)$item->armah_module_not)
     ...
     * @return string[]
     */
    private function getInstalledArmahExtensions(): array
    {
        if (!$this->armahModules) {
            $modules = $this->moduleList->getNames();
            ...
            $modules = array_filter(
                ...
                static function ($item) {
                    return strpos($item, 'Armah_') !== false;
                }
            );
            $this->armahModules = $modules;
        }

        return $this->armahModules;
    }
    ...
    private function validateByExtension(string $extensions, bool $allModules = false): bool
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $allModules ? $this->getAllExtensions() : $this->getInstalledArmahExtensions();
    ...
    private function validateByNotInstalled(string $extensions): bool
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $this->getInstalledArmahExtensions();
     ...
     * @return bool
     */
    private function validateByArmahCount($counts): bool
    {
        $result = true;
        ...
        if ($countString) {
            ...
            $armahModules = $this->getInstalledArmahExtensions();
            $dependModules = $this->getDependModules($armahModules);
            $armahModules = array_diff($armahModules, $dependModules);

            $armahCount = count($armahModules);

            if ($armahCount
                && (
                    in_array($armahCount, $arrCounts, false) // non strict
                    || ($moreThan && $armahCount >= $moreThan)
                )
            ) {
                $result = true;
            }
    ...
    /**
     * @param string[] $armahModules
     *
     * @return array
     */
    private function getDependModules(array $armahModules): array
    {
        $depend = [];
        $result = [];
        $dataName = [];
        foreach ($armahModules as $module) {
            ...
            if (isset($data['require']) && is_array($data['require'])) {
                foreach ($data['require'] as $requireItem => $version) {
                    if (strpos($requireItem, 'armah') !== false) {
                        $depend[] = $requireItem;
                    }
                }
            }
