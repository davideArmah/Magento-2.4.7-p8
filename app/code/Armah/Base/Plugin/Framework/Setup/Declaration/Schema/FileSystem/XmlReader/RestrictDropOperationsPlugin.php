...
namespace Armah\Base\Plugin\Framework\Setup\Declaration\Schema\FileSystem\XmlReader;

use Magento\Framework\Config\FileIterator;
use Magento\Framework\Config\FileResolverByModule;
use Armah\Base\Model\SysInfo\Provider\Module\Dir\Reader;
...
class RestrictDropOperationsPlugin
{
    ...
    public function afterGet(FileResolverByModule $subject, array $result, $filename, $scope): array
    {
        if ($filename === 'db_schema.xml'
            ...
            $armahDisabledModules = $this->moduleReader->getConfigurationFiles($filename, 'Armah', false)->toArray();
            $allModulesIterator = $this->moduleReader->getConfigurationFiles($filename);

            $sortedResult = $this->buildSorted(
                $allModulesIterator,
                [$result, $armahDisabledModules]
            );
