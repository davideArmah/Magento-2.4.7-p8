...
use Armah\Base\Model\ArmahMenu\ArmahConfigItemsProvider;
...
class Tabs extends Template
{
     ...
     * @var string
     */
    protected $_template = 'Armah_Base::config/armah_tabs.phtml';
    ...
    private function getActiveExtensions(): array
    {
        ...
        if ($armahTab = $this->configItemsProvider->getArmahConfigChildrenNode()) {
            foreach ($armahTab as $section) {
    ...
    public function extractBaseTabs(array &$extensions): array
    {
        ...
        if (isset($extensions['Armah_Base'])) {
            $result[] = $extensions['Armah_Base']; //separate base module from all
            unset($extensions['Armah_Base']);
