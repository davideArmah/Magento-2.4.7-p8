...
namespace Armah\Sorting\Observer\System;
...
class ConfigChanged implements ObserverInterface
{
        ...
        if ($catalogValue != $moduleValue) {
            switch ($name) {
                case 'admin_system_config_changed_section_catalog':
                    $this->saveArmahValue($catalogValue, $store);
                    break;
                case 'admin_system_config_changed_section_armahsorting':
     ...
     * @param $store
     */
    private function saveArmahValue($value, $store)
    {
        $this->saveConfig('armahsorting/default_sorting/category_1', $value, $store);
    }
