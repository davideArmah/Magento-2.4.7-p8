...
namespace Armah\Sorting\Setup\Patch\Data;
...
class UpdateDefaultSearch implements DataPatchInterface
{
    ...
    {
        return [
            \Armah\Sorting\Setup\Patch\Data\RenameLabelsField::class
        ];
    }
    ...
    public function apply(): UpdateDefaultSearch
    {
        if ($this->isCanProceed()) {
            ...
            foreach ($rows as $row) {
                $updateData[] = [
                    'value' => $row['value'],
                    'path'  => 'armahsorting/default_sorting/category_1',
                    'scope' => $row['scope'],
    ...
    {
        $select = $this->prepareSelect('armahsorting/default_sorting/category_1');
