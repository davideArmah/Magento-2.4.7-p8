...
class Information extends Fieldset
{
     ...
     * @var string
     */
    private $userGuide = 'https://armah.it/docs/doku.php?id=magento_2:improved-sorting';
    ...
    public function render(AbstractElement $element)
    {
        ...
        $this->setContent(__('Please update Armah Base module. Re-upload it and replace all the files.'));

        $this->_eventManager->dispatch(
            'armah_base_add_information_content',
            ['block' => $this]
        );
        ...
        $html = str_replace(
            'armah_information]" type="hidden" value="0"',
            'armah_information]" type="hidden" value="1"',
            $html
        );
    ...
    public function getAdditionalModuleContent()
    {
        if ($this->moduleManager->isEnabled('Magento_GraphQl')
            && !$this->moduleManager->isEnabled('Armah_SortingGraphQl')
        ) {
            $result[] = [
                'type' => 'message-notice',
                'text' => __('Enable improved-sorting-graphql module to '
                    ...
                    . 'composer require armah/improved-sorting-graphql')
            ];
        }

        if ($this->moduleManager->isEnabled('Yotpo_Yotpo')
            && !$this->moduleManager->isEnabled('Armah_Yotpo')
        ) {
            $result[] = [
                'type' => 'message-notice',
                'text' => __('Enable armah/yotpo module to '
                    . 'activate Yotpo and Sorting. '
                    . 'Please, run the following command in the SSH: '
                    . 'composer require armah/yotpo')
            ];
        }
    ...
    private function getM245Notice(): ?array
    {
        ...
        if (!$this->moduleManager->isEnabled('Armah_Mage245Fix')
            ...
            $notice = [
                ...
                'text' => __('Enable the module-mage-2.4.5-fix module for the extension to function correctly. '
                    . 'Please, run the following command in the SSH: composer require armah/module-mage-2.4.5-fix')
            ];
        }
        ...
        if ($this->moduleManager->isEnabled('Armah_Mage245Fix')
            ...
            $notice = [
                ...
                'text' => __('Considering your current Magento version, please disable the module-mage-2.4.5-fix '
                    ...
                    . 'Please, run the following command in the SSH: php bin/magento module:disable Armah_Mage245Fix')
            ];
        }
