...
abstract class AbstractIndexMethod extends AbstractMethod implements IndexedMethodInterface
{
    ...
    public function beforeReindex()
    {
        try {
            ...
            throw new CouldNotDeleteException(__('Error while clear index armah sorting method: '), $e);
        }
    ...
    {
        return 'armah_sorting_' . $this->getMethodCode();
    }
    ...
    public function apply($collection, $direction)
    {
        try {
            ...
            $this->logger->warning(
                'Failed on join table for armah sorting method: ' . $e->getMessage(),
                ['method_code' => $this->getMethodCode()]
            );
        } catch (\Exception $e) {
